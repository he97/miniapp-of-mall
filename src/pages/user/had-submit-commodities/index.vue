  ...
  <div>
      <div>
        ...
        <div v-for="(i,value) in commodities" :key="value">
          ...
          >
            <view slot="footer">
              <van-button size="mini" round @click="onSell(i.commodityId)">审核</van-button>
              <van-button size="mini" round @click="underCarriage(i.commodityId)">下架</van-button>
            </view>
          </van-card>
        </div>
        <van-dialog
...
import {GET_SUBMIT, ALTER_COMMODITY_STATUS} from '@/utils/api'
...
export default {
  ...
  methods: {
    async onSell (id) {
      wx.navigateTo({
        url: '/pages/commodity/submit/main?commodityId=' + id
      })
      // const resp = await request(
      //   ALTER_COMMODITY_STATUS,
      //   'POST',
      //   {
      //     'commodityId': id,
      //     'type': 'WAIT_CHECK'

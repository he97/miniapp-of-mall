  ...
  <div>
      <div>
        ...
        <div v-for="(i,value) in commodities" :key="value">
          ...
          >
            <view slot="footer">
              <van-button size="mini" @click="returnTransaction(i)">退款</van-button>
              <van-button size="mini" @click="finishTransaction">确认收货</van-button>
            </view>
          </van-card>
        </div>
      </div>
...
import {GET_BOUGHT, ADD_CART, START_CANCEL_TRAN} from '@/utils/api'
...
export default {
  ...
  methods: {
    async returnTransaction (i) {
      const returnTranResp = await request(
        START_CANCEL_TRAN,
        'POST',
        {

  ...
  <div>
      <div>
        ...
        <div v-for="(i,value) in commodities" :key="value">
          <van-card
            :tag="i.commodityStatus"
            :num="i.count"
            :price="i.price"
            :desc="i.commodityInformation"
            :title="i.commodityName"
            :thumb="i.commodityFirstPictureUrl"
            :thumb-link="'/pages/user/sell-commodity-info/main?id=' + i.commodityId"
            @click="toSellInfo(i.commodityId)"
          >
            <view slot="footer">
              <div v-if="i.commodityStatus === '等待是否取消订单' " >
              <van-button size="mini" @click="agreereturnTransaction(i)">同意退款</van-button>
              <van-button size="mini" @click="rejectreturnTransaction(i)">拒绝退款</van-button>
              </div>
            </view>
          </van-card>
        </div>
...
import {GET_SELL, ADD_CART, HANDLE_CANCEL_TRAN} from '@/utils/api'
...
export default {
  ...
  methods: {
    async agreereturnTransaction (i) {
      const agreeReturnResp = await request(
        HANDLE_CANCEL_TRAN,
        'POST',
        {
          'type': 'AGREE',
      ...
      console.info('agreeReturnResp', agreeReturnResp)
    },
    async rejectreturnTransaction (i) {
      const rejectReturnResp = await request(
        HANDLE_CANCEL_TRAN,
        'POST',
        {
          'type': 'REJECT',
      ...
      console.info('rejectReturnResp', rejectReturnResp)
    },
    toSellInfo (id) {
      wx.navigateTo({
        url: '/pages/user/sell-commodity-info/main?id=' + id
      })
    },
    async addCart (id) {
